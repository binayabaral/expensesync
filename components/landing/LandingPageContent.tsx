    ...
    <div className='relative min-h-screen bg-background text-foreground'>
      ...
      <section className='relative overflow-hidden'>
        ...
        <div className='relative mx-auto max-w-6xl px-4 sm:px-6 py-24 sm:py-36 text-center'>
          ...
          <motion.h1
            className='mx-auto max-w-3xl text-4xl font-bold tracking-tight sm:text-5xl md:text-6xl lg:text-7xl'
            initial={{ opacity: 0, y: 24 }}
            animate={{ opacity: 1, y: 0 }}
      ...
      <section id='features' className='border-t border-border/50 bg-muted/10'>
        <div className='mx-auto max-w-6xl px-4 sm:px-6 py-24'>
          ...
          <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3'>
            {[
              { icon: FaReceipt, title: 'Expense Tracking', description: 'Log every transaction with categories, payees, and notes. Filter and search across your full history.' },
              { icon: FaArrowsRotate, title: 'Recurring Payments', description: 'Log recurring bills and subscriptions and get push notifications the day before and the day they are due. Never miss a payment.' },
              { icon: FaWallet, title: 'Accounts & Cards', description: 'Manage multiple bank accounts, credit cards, and loans. See balances at a glance.' },
              { icon: FaTags, title: 'Categories & Payees', description: 'Organise spending with custom categories and payees. Understand where your money actually goes.' },
              { icon: FaChartLine, title: 'Spending Trends', description: 'Visual charts break down your spending by category and period so you can spot patterns instantly.' },
              { icon: FaMoneyBillTransfer, title: 'Transfers', description: 'Record money moved between your own accounts without double-counting it as income or expense.' },
              { icon: FaHeart, title: 'Financial Health', description: 'Know where you stand. Net worth, savings rate and financial health in one view.', pro: true },
              { icon: FaCoins, title: 'Asset Tracking', description: 'Live pricing of stocks, gold and silver. Track physical and financial assets to monitor your net worth.', pro: true },
              { icon: FaScaleBalanced, title: 'Bill Splitting', description: 'Split expenses with friends and groups. Track who owes what and settle balances with ease.', pro: true },
              { icon: FaRobot, title: 'AI Advisor', description: 'AI looks at your real spending data and tells you exactly what to fix. Not generic advice, your actual numbers.', pro: true },
              { icon: FaLayerGroup, title: 'AI Financial Organizer', description: 'Get a payday plan, monthly calendar and budget breakdown built around your actual income and bills.', pro: true }
            ].map((feature, i) => (
              <motion.div
                key={feature.title}
                initial={{ opacity: 0, y: 24 }}
                ...
                className='h-full'
              >
                <Card className={`h-full group border-border/60 bg-card/50 hover:bg-card transition-colors duration-200 ${feature.pro ? 'hover:border-primary/40' : 'hover:border-primary/30'} relative overflow-hidden`}>
                  {feature.pro && (
            ...
            <div className='relative'>
              ...
              <Card className='border-border/60 shadow-xl relative'>
                <CardContent className='p-6'>
                  ...
                  <div className='text-xs text-muted-foreground uppercase tracking-wide mb-2'>Members</div>
                  <div className='flex flex-wrap gap-2 mb-5'>
                    {[
                      ...
                      <div key={m.name} className='flex items-center gap-1.5 border border-border/60 rounded-full px-2.5 py-1'>
                        <div className={`h-5 w-5 rounded-full flex items-center justify-center text-[9px] font-bold ${m.color}`}>{m.initials}</div>
                  ...
                  <div className='text-xs text-muted-foreground uppercase tracking-wide mb-2'>Recent expenses</div>
                    ...
                    {[
                      ...
                      <div key={exp.name} className='rounded-md border border-border/50 p-2.5 bg-muted/20'>
                        ...
                        <div className='flex flex-wrap gap-1'>
                          {exp.shares.map(s => (
                            <span key={s} className='text-[9px] bg-background border border-border/50 rounded px-1.5 py-0.5'>{s}</span>
                          ))}
                        </div>
                      </div>
                    ))}
        ...
        <div className='mx-auto max-w-6xl px-4 sm:px-6 py-24'>
          <div className='grid gap-12 lg:grid-cols-2 items-center'>
              ...
              <Card className='border-border/60 shadow-xl relative'>
                <CardContent className='p-6'>
                  ...
                  <div className='space-y-2.5'>
                    {[
                      { color: 'border-l-destructive bg-destructive/5', badge: 'bg-destructive/15 text-destructive', cat: 'savings', title: 'Emergency fund below target', desc: 'Current liquid reserves cover ~6 weeks. Aim for 3 months.' },
                      { color: 'border-l-yellow-500 bg-yellow-500/5', badge: 'bg-yellow-500/15 text-yellow-600', cat: 'spending', title: 'Dining spend up 28% this month', desc: 'NPR 12,400 vs your NPR 9,700 average. Review recent restaurant transactions.' },
                      { color: 'border-l-green-500 bg-green-500/5', badge: 'bg-green-500/15 text-green-600', cat: 'investments', title: 'SIP on track', desc: 'Monthly SIP deductions running consistently. No action needed.' }
                    ].map(rec => (
            ...
            <FadeUp delay={0.15} className='order-1 lg:order-2'>
              <Badge variant='secondary' className='mb-4'>AI features</Badge>
              <h2 className='text-3xl font-bold tracking-tight sm:text-4xl mb-5'>
                Your finances,{' '}
                <span className='bg-linear-to-r from-primary to-primary/60 bg-clip-text text-transparent'>
                  explained by AI.
      ...
      <section className='border-t border-border/50 bg-muted/10'>
        <div className='mx-auto max-w-6xl px-4 sm:px-6 py-24'>
          ...
          <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-4'>
            {[
              { icon: FaWallet, title: 'NPR Native', desc: 'Built around Nepali Rupees. No awkward conversion just to log a daily expense.' },
              { icon: FaChartLine, title: 'NEPSE Stocks', desc: 'Track your stock portfolio with live NEPSE prices updated twice daily on trading days.' },
              { icon: FaCoins, title: 'Gold in Tola', desc: 'Buy and sell gold by the tola in 22K and 24K, tracked with live rates from local sources.' },
              { icon: FaReceipt, title: 'Nepali Bank Quirks', desc: 'Credit card statements that do not close neatly at month end? Handled exactly as Nepali banks work.' }
            ].map((item, i) => (
              <motion.div
                key={item.title}
                initial={{ opacity: 0, y: 24 }}
                ...
                whileHover={{ y: -4 }}
                className='rounded-xl border border-border/60 bg-card/50 p-6 hover:bg-card transition-colors cursor-default'
              >
      ...
      <section id='how-it-works' className='border-y border-border/50 bg-muted/20'>
        <div className='mx-auto max-w-6xl px-4 sm:px-6 py-24'>
          ...
          <div className='grid gap-8 sm:grid-cols-3 relative'>
            ...
            {[
              { step: '01', title: 'Add your accounts', description: 'Add your bank accounts, credit cards, and loans to get a complete picture of your finances.' },
              { step: '02', title: 'Track transactions', description: 'Log income and expenses with categories and notes. Enter them as they happen.' },
              { step: '03', title: 'Split & settle', description: "Create bill-split groups for trips, shared housing, or any shared expense. Track who owes what and settle up when you're ready." }
            ].map((step, i) => (
              <motion.div
                key={step.step}
        ...
        <div className='mx-auto max-w-6xl px-4 sm:px-6 py-24'>
          ...
          <div className='mx-auto max-w-3xl grid gap-6 sm:grid-cols-2'>
            <FadeUp delay={0.05} className='h-full'>
              <Card className='border-border/60 relative overflow-hidden h-full'>
                <CardContent className='p-8 flex flex-col h-full'>
                  <div className='flex-col justify-between'>
                    <div className='mb-8'>
                      ...
                      <div className='text-sm text-muted-foreground mt-2'>Core features, forever free</div>
                    </div>
                    <ul className='space-y-3 mb-8'>
                      {['Unlimited transactions', 'Multiple accounts & cards', 'Recurring payment tracking', 'Categories & payees', 'Spending trends & charts', 'Dark & light mode'].map(f => (
            ...
            <FadeUp delay={0.12} className='h-full'>
              <Card className='border-primary/40 shadow-lg relative overflow-hidden h-full'>
                ...
                <CardContent className='p-8'>
                  <div className='mb-8'>
                    ...
                    <div className='text-sm text-muted-foreground mt-2'>Less than NPR 17 per month. Early access pricing.</div>
                  </div>
                  <ul className='space-y-3 mb-6'>
                    {[
                      'Everything in Free',
                      'Financial health dashboard',
                      'Asset tracking with live pricing',
                  ...
                  <div className='flex items-center gap-2 mb-8 rounded-md bg-muted/50 border border-border/50 px-3 py-2'>
                    ...
                    <p className='text-xs text-muted-foreground'>More Pro features coming: bulk import, export and more.</p>
                  </div>
      ...
      <section id='faq' className='mx-auto max-w-6xl px-4 sm:px-6 py-24'>
        ...
        <div className='mx-auto max-w-2xl space-y-3'>
          {[
            { q: 'How much does XpenseSync cost?', a: 'XpenseSync is free right now. When paid plans launch, early users will get plenty of notice and will likely get a discount for joining early.' },
            { q: 'Can I manage multiple bank accounts and credit cards?', a: 'You can add as many accounts as you need. Savings accounts, current accounts, credit cards, loans, physical assets. Everything in one place.' },
            ...
            >
              <details className='group rounded-lg border border-border/60 bg-card/50 open:bg-card transition-colors duration-200'>
                <summary className='flex items-center justify-between gap-4 px-5 py-4 cursor-pointer list-none select-none font-medium text-sm'>
                  {item.q}
                  <span className='shrink-0 text-muted-foreground group-open:hidden'><FaPlus className='h-3 w-3' /></span>
                  <span className='shrink-0 text-muted-foreground hidden group-open:block'><FaMinus className='h-3 w-3' /></span>
                </summary>
      ...
      <section className='border-y border-border/50'>
        <div className='mx-auto max-w-6xl px-4 sm:px-6 py-14'>
          <div className='grid gap-8 sm:grid-cols-2 lg:grid-cols-4'>
            {[
              { icon: FaShieldHalved, title: 'No bank sync', desc: 'Your banking credentials never touch our servers. Nothing is connected automatically.' },
              { icon: FaEyeSlash, title: 'No ads, ever', desc: 'XpenseSync does not run ads or sell your financial data to anyone, period.' },
              { icon: FaPencil, title: 'Manual by design', desc: 'Entering transactions yourself keeps you aware of every rupee. That awareness is the point.' },
              { icon: FaLock, title: 'Your data, yours', desc: 'Stored securely in the cloud. You can export or delete everything at any time.' }
            ].map((item, i) => (
              <motion.div
                key={item.title}
